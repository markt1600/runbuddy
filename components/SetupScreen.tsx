...
// the partner for duo mode, start. The run settings (music, target, chatter,
// auto-pause…) are summarised in one line above Start and unfold on
// "Change" — they are remembered between runs, so most days nobody touches
// them.
...
const MUSIC_SHORT: Record<MusicSource, string> = {
  spotify: "Spotify",
...
interface Props {
  ...
  /** The named Start button at the foot of the screen. */
  onStart: () => void;
}
...
export default function SetupScreen({
  ...
  // "Change" unfolds the settings below the picker; bring them into view.
  useEffect(() => {
  ...
  const targetLine =
    mode === "distance"
      ...
      : mode === "time"
        ...
        : mode === "pace"
          ? `${formatTargetPace(targetPaceSec)} /km target`
          : "No target";
  const chatterLine =
    chattiness <= CHATTINESS_MIN ? chattinessLabel(chattiness) : `${chattinessLabel(chattiness)} chatter`;
  const summaryLine = [
    targetLine,
    chatterLine,
    MUSIC_SHORT[music],
  ...
  const startNames = duoMode
    ? `${PERSONAS.ahbeng.shortName} & ${PERSONAS.ahlian.shortName}`
    : PERSONAS[personaId].shortName;
  ...
  return (
    <div className="fade-in setup">
      ...
      {showSettings && (
        <div className="setup-settings" ref={settingsRef}>
      ...
      <div className="footer-cta setup-foot">
        <div className="setup-summary">
          <span title={summaryLine}>{summaryLine}</span>
          <button className="link-btn" onClick={() => setShowSettings((s) => !s)}>
            {showSettings ? "Done" : "Change"}
          </button>
        </div>
        <button className="cta setup-start" onClick={onStart}>
          Start with {startNames}
        </button>
      </div>
    </div>
  );
