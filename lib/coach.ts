...
const FRESH_PHRASE_CHANCE = 0.35; // odds an anecdote slot asks the API for new material
...
export class CoachEngine {
  ...
  tick(stats: RunStats) {
    ...
    // 1. Kilometre milestones (highest priority)
    ...
    if (now >= this.nextAnecdoteAt) {
      this.nextAnecdoteAt = now + between(ANECDOTE_GAP_MS);
      if (Math.random() < FRESH_PHRASE_CHANCE) {
  ...
  private async sayFresh(category: PhraseCategory, stats: RunStats) {
    try {
