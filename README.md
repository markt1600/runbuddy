...
- **Per-persona voice settings** — speed (applies at render time) and playback
  level (applies at play time, no re-render), both in admin. Level covers every
  path a phrase can take: pre-rendered clips, live-generated ones, the device
  fallback, the admin preview and the post-run summary line.
